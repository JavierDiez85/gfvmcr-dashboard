    ...
    switch (name) {
      ...
      case 'project_cash_flow': {
        ...
        for (const key of ['gf_cred_end', 'gf_cred_dyn']) {
          ...
          _processCredits(raw).filter(c => c.estado === 'Activo').forEach(c => { creditIncome += c.pagoFijo || 0; });
        }
        let tpvMonthlyAvg = 0;
        try {
      ...
      case 'get_alerts': {
        ...
        try {
          ...
          if (Array.isArray(terminals)) terminals.forEach(t => { if ((t.dias_sin_uso || 0) > 15) alerts.push({ tipo: 'TERMINAL_INACTIVA', prioridad: 'BAJA', empresa: 'Salem', cliente: t.cliente, mensaje: `Terminal ${t.terminal_id} sin actividad hace ${t.dias_sin_uso} días` }); });
        } catch(e) { /* skip */ }
