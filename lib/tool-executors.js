...
function _credPeriodStatus(credit, row) {
  ...
  if (totalPagado > 0) {
    // Misma tolerancia dinámica que credit-engine.js: max($0.10, 0.01% del pago)
    const _tol = Math.max(0.10, (row.pago||0) * 0.0001);
    return totalPagado >= (row.pago||0) - _tol ? 'PAGADO' : 'PARCIAL';
  }
    ...
    switch (name) {
      ...
      case 'project_cash_flow': {
        ...
        for (const key of ['gf_cred_end', 'gf_cred_dyn']) {
          ...
          // Solo interés mensual — el capital es recuperación de cartera, no ingreso del grupo
          _processCredits(raw).filter(c => c.estado === 'Activo').forEach(c => {
            const r = (c.tasa || 0) / 100 / 12;
            creditIncome += (c.saldoActual || c.monto || 0) * r;
          });
        }
        let tpvMonthlyAvg = 0;
        try {
        ...
        for (let i = 1; i <= months; i++) {
          ...
          projections.push({
            ...
            interesCreditos: +creditIncome.toFixed(2), ingresosTPV: +tpvMonthlyAvg.toFixed(2),
        ...
        return { fecha_analisis: new Date().toISOString().slice(0,10), nota: 'Proyección basada en promedios históricos. interesCreditos = saldo_actual × tasa_mensual (solo interés, sin recuperación de capital).', meses_proyectados: months, proyeccion: projections };
      }
